	...
	//	var tests_link = testSuite_link.getElementsByTagName("TestScenario");
	   ...
	   //Madhavan -end
		...
		//	var replayUrl = "../replayshortcuts/Replayshortcuts.html?xml=" + xmlFile + "&doc=" + xmlDoc+
			...
			//Madhavan -start
			var str = (tests.length != (v+1) ) ? getAttr(tests[v+1], "id"): 'Test';
			if((testReportCaseRequirementIdTag != null) && 
			      (scenarioName[v] != str) )
			{
			   var intime = timeToMillisecs(getAttr(test, "executionTime"));
			   if(pastMS == 0) {
			      pastMS = intime;
			   }
			   else 
			   {
			      pastMS = intime - pastMS;
			   }
			//aTag=textDescTag(aTag);
			sno++;    
			var tableRow = new TableRow();
			...
			tableRow.insertRecord(testCaseDescription);
			tableRow.insertRecord(data);
			tableRow.insertRecord(textTag(msToTime(pastMS)));
			pastMS = intime;
			tableRow.insertRecord(fullReportViewTag);
			tableRow.insertRecord(replayScreenshotsTag);
			//Madhavan -end
